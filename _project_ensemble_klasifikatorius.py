...
def train_and_evaluate(config: Config):
    ...
    # Atskiri modeliai be pipeline, kad tiksliai valdytume SMOTE
    ...
    all_metrics["ensemble"] = evaluate_binary_classifier(
        ensemble_model, X_test_features, y_test, title="Soft Voting Ensemble"
    )
...
def main():
    ...
    plot_class_distribution(
        extra_data["y"],
        os.path.join(config.output_dir, "prt2_class_distribution.png")
    )

    plot_class_distribution_train_before_after(
        extra_data["y_train_before_smote"],
        extra_data["y_train_after_smote"],
        os.path.join(config.output_dir, "prt2_class_distribution_smote.png")
    )
    ...
    for model_name, model_bundle in trained_models.items():
        ...
        plot_confusion_matrix_figure(
            ...
            os.path.join(config.output_dir, f"prt2_confusion_matrix_{safe_name}.png")
        )
    ...
    plt.legend()
    plt.tight_layout()
    roc_path = os.path.join(config.output_dir, "prt2_roc_curves.png")
    ...
    plot_metrics_comparison(
        metrics,
        os.path.join(config.output_dir, "prt2_metrics_comparison.png")
    )
