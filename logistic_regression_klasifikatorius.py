...
class Config:
    ...
    random_state: int = 42
    model_output_path: str = "logistic_regression_model.joblib"
    metadata_output_path: str = "logistic_regression_metadata.json"
...
def build_logistic_model(config: Config):
...
def main():
    ...
    config = Config(
        data_dir=data_dir,
        model_output_path=os.path.join(script_dir, "logistic_regression_model.joblib"),
        metadata_output_path=os.path.join(script_dir, "logistic_regression_metadata.json")
    )

    print("\nPradedamas Logistic Regression modelio mokymas...\n")
    ...
    model = build_logistic_model(config)
    model.fit(X_train, y_train)

    metrics = evaluate_binary_classifier(model, X_test, y_test, title="Logistic Regression")
